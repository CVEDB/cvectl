...
func TestChecks_ParsePackages(t *testing.T) {
	...
	// packages.log is an example output of packages that were built by melange, eg cvedb/os
...
func TestChecks_downloadCurrentAPK(t *testing.T) {
	...
	data, err := os.ReadFile(filepath.Join("testdata", "hello-cvedb-2.12-r1.apk"))
	assert.NoError(t, err)

	// create a test server to download the test apk from
	server := httptest.NewServer(http.HandlerFunc(func(rw http.ResponseWriter, req *http.Request) {
		// Test request parameters
		assert.Equal(t, req.URL.String(), "/hello-cvedb-2.12-r1.apk")
	...
	err = downloadCurrentAPK(server.Client(), server.URL+"/APKINDEX.tar.gz", "hello-cvedb-2.12-r1.apk", dir)
	assert.NoError(t, err)
