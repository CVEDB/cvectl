...
import (
	...
	"github.com/cvedb/cvectl/pkg/apk"
	"github.com/cvedb/cvectl/pkg/lint"
	"github.com/cvedb/cvectl/pkg/tar"
...
func NewSoName() *SoNameOptions {
	o := &SoNameOptions{
		...
		Logger: log.New(log.Writer(), "cvectl check so-name: ", log.LstdFlags|log.Lmsgprefix),
	}
...
func (o *SoNameOptions) diff(newPackageName string, newAPK NewApkPackage) error {
	dirExistingApk, err := os.MkdirTemp("", "cvectl-apk-*")
	if err != nil {
		return fmt.Errorf("failed to create temporary dir: %w", err)
	}
	defer os.RemoveAll(dirExistingApk)

	dirNewApk, err := os.MkdirTemp("", "cvectl-apk-*")
	if err != nil {
		return fmt.Errorf("failed to create temporary dir: %w", err)
	}
