...
func NewGraph(ctx context.Context, pkgs *Packages, options ...GraphOptions) (*Graph, error) {
	...
	for _, c := range pkgs.Packages() {
		...
		// cvedb has a policy for environment packages not to use a package to fulfull a dependency, if that package is myself.
...
// In general, cvedb/os does *not* allow self to resolve for build environment,
