...
The ruby subcommand is intended to work with all ruby packages inside the cvedb
...
This command takes a path to the cvedb/os repository as an argument. The
...
cvectl ruby code-search . --ruby-version 3.2 --search-term 'language:ruby racc'

# Check if all ruby-3.2 packages in the current directory can be upgraded to ruby-3.3
cvectl ruby check-upgrade . --ruby-version 3.2 --ruby-upgrade-version 3.3
`,
