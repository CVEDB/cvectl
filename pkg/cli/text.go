...
func cmdText() *cobra.Command {
	...
	text.Flags().StringSliceVarP(&extraKeys, "keyring-append", "k", []string{"https://packages.cvedb.dev/os/cvedb-signing.rsa.pub"}, "path to extra keys to include in the build environment keyring")
	text.Flags().StringSliceVarP(&extraRepos, "repository-append", "r", []string{"https://packages.cvedb.dev/os"}, "path to extra repositories to include in the build environment")
	return text
}
