...
func (o ReleaseOptions) Release(ctx context.Context) error {
	...
	current, err := cvedbgit.GetVersionFromTag(o.Dir, 1)
	...
	err = cvedbgit.CreateTag(o.Dir, next.Original())
	if err != nil {
		return fmt.Errorf("failed to create tag %s: %w", next.Original(), err)
	}

	// push new tag
	err = cvedbgit.PushTag(o.Dir, next.Original())
	if err != nil {
		return err
	}
...
func (o ReleaseOptions) createGitHubRelease(ctx context.Context, v string) error {
	...
	gitURL, err := cvedbgit.GetRemoteURL(repo)
	if err != nil {
