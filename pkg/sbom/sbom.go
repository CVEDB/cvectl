...
const cpeSourceCvectl cpe.Source = "cvectl"
...
func Generate(ctx context.Context, inputFilePath string, f io.Reader, distroID string) (*sbom.SBOM, error) {
	...
	tempDir, err := os.MkdirTemp("", "cvectl-sbom-*")
	if err != nil {
...
func generateCPEs(p pkg.Package) []cpe.CPE {
	...
	// 	upstream, but it's more obvious how we can address this in cvectl for our
	// 	purposes.
	//
