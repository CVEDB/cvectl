...
func Test_getApkPackages(t *testing.T) {
	...
	cvedbPackages, err := c.GetApkPackages()
	assert.NoError(t, err)
	assert.Equal(t, "x86_64", cvedbPackages["pkgconf-doc"].Arch)
	assert.Equal(t, "5.2_rc4-r0", cvedbPackages["bash-doc"].Version)
}
...
func Test_ParseApkIndex(t *testing.T) {
	...
	cvedbPackages, err := ParseUnpackedApkIndex(f)
	assert.NoError(t, err)
	assert.Equal(t, "4.33-r0", cvedbPackages["libev-doc"].Version)
	assert.Equal(t, "0.19.0-r13", cvedbPackages["tini"].Version)
}
