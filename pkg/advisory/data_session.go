...
func NewDataSession(ctx context.Context, opts DataSessionOptions) (*DataSession, error) {
	...
	tempDir, err := os.MkdirTemp("", "cvectl-advisory-data-session-*")
	if err != nil {
	...
	u := uuid.New()
	branchName := fmt.Sprintf("cvectl-data-session-%s", u)
...
const pullRequestBody = "This PR was created using the `cvectl adv guide` command."
