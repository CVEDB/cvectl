...
import (
	...
	"github.com/cvedb/cvectl/pkg/internal/errorhelpers"
	"gopkg.in/yaml.v3"
)

// SchemaVersion is the latest known schema version for advisory documents.
// Cvectl can only operate on documents that use a schema version that is
...
func (doc Document) ValidateSchemaVersion() error {
	...
	if docSchemaVersion.GreaterThan(currentSchemaVersion) {
		return fmt.Errorf("document schema version %q is newer than the latest known schema version %q; if %q is supported by a later version of cvectl, please update cvectl and try this again", doc.SchemaVersion, SchemaVersion, doc.SchemaVersion)
	}
	...
	if docMajorNumber < currentMajorNumber {
		return fmt.Errorf("document schema version %q is too old to operate on with this version of cvectl, document must use at least schema version \"%d\"", doc.SchemaVersion, currentMajorNumber)
	}
