...
import (
	...
	"github.com/cvedb/cvectl/pkg/yam"
	...
	"github.com/cvedb/cvectl/pkg/melange"
	"github.com/cvedb/cvectl/pkg/update/deps"
	cvedbversions "github.com/cvedb/cvectl/pkg/versions"
...
const (
	...
	cvedbImage            = `
<p align="center">
  <img src="https://raw.githubusercontent.com/cvedb/.github/b535a42419ce0edb3c144c0edcff55a62b8ec1f8/profile/cvedb-logo-light-mode.svg" />
</p>
`
)

// New initialise including a map of existing cvedbos packages
...
func (o *Options) Update(ctx context.Context) error {
	...
	for i := 0; i < o.MaxRetries; i++ {
		...
		tempDir, err := os.MkdirTemp("", "cvectl")
		if err != nil {
...
func (o *Options) updateGitPackage(ctx context.Context, repo *git.Repository, packageName string, newVersion NewVersionResults, ref plumbing.ReferenceName) (string, error) {
	...
	// for now cvedb is using a Makefile, if it exists check if the package is listed and update the version + epoch if it is
...
func (o *Options) createBranch(repo *git.Repository) (plumbing.ReferenceName, error) {
	...
	branchName := plumbing.NewBranchReferenceName(fmt.Sprintf("cvectl-%s", name))
...
func (o *Options) commitChanges(repo *git.Repository, packageName, latestVersion string) error {
	...
	if latestVersion != "" {
		...
		commitMessage = "Updating cvedb packages"
	}
...
func (o *Options) getPackagesToUpdate(latestVersions map[string]NewVersionResults) (map[string]NewVersionResults, error) {
	...
	for packageName, v := range latestVersions {
		...
		c := pc.Config
		currentVersionSemver, err := cvedbversions.NewVersion(c.Package.Version)
		if err != nil {
			return nil, fmt.Errorf("failed to create a version from package %s: %s: %w", c.Package.Name, c.Package.Version, err)
		}

		latestVersionSemver, err := cvedbversions.NewVersion(v.Version)
		if err != nil {
		...
		if currentVersionSemver.LessThan(latestVersionSemver) {
			o.Logger.Println(
				color.GreenString(
					fmt.Sprintf("there is a new stable version available %s, current cvedb version %s, new %s",
...
func (o *Options) containsOldVersion(packageName, latestVersionStr, titleVersionStr, title string) bool {
	...
	currentVersion, err := cvedbversions.NewVersion(titleVersionStr)
	if err != nil {
		// ignore if we can't create a real version
		return false
	}

	latestVersion, err := cvedbversions.NewVersion(latestVersionStr)
	if err != nil {
