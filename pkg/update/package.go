...
import (
	...
	rwfsOS "github.com/cvedb/cvectl/pkg/configs/rwfs/os"
	...
	cvedbgit "github.com/cvedb/cvectl/pkg/git"
...
func (o *PackageOptions) UpdatePackageCmd(ctx context.Context) error {
	...
	gitAuth, err := cvedbgit.GetGitAuth(o.TargetRepo)
	if err != nil {
...
func (o *PackageOptions) updateAdvisories(ctx context.Context, repo *git.Repository) error {
	...
	gitURL, err := cvedbgit.GetRemoteURLFromDir(currentDir)
	if err != nil {
		return err
	}

	gitAuth, err := cvedbgit.GetGitAuth(gitURL.RawURL)
	if err != nil {
		return fmt.Errorf("failed to get git auth: %w", err)
	}
	...
	previous, err := cvedbgit.GetVersionFromTag(tempDir, 2)
	if err != nil {
	...
	// run the equivalent of `cvectl advisory create ./foo.melange.yaml --vuln 'CVE-2022-31130' --status 'fixed' --fixed-version '7.5.17-r1'`
...
//	}
//
//	remoteURL, err := cvedbgit.GetRemoteURL(r)
//	if err != nil {
//		return "", err
//	}
//
...
func (o *PackageOptions) addCommit(repo *git.Repository, fixes []string) error {
	...
	commitOpts.Author = cvedbgit.GetGitAuthorSignature()

	if o.UseGitSign {
		err := cvedbgit.SetGitSignOptions(wt.Filesystem.Root())
		if err != nil {
