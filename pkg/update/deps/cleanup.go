...
// ContainsGoBumpPipeline checks whether there is a gobump in the cvedb package definition.
...
func CleanupGoBumpDeps(doc *yaml.Node, updated *config.Configuration, tidy bool, mutations map[string]string) error {
	tempDir, err := os.MkdirTemp("", "cvedbbump")
	if err != nil {
	...
	if pipelineNode == nil {
		return fmt.Errorf("pipeline node not found in the Cvedb definition")
	}
