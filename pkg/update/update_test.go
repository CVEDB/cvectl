...
func TestMonitorService_updatePackagesGitRepository(t *testing.T) {
	...
	r := setupTestCvedbRepo(t, dir, server.URL)
	...
	o.PackagesToUpdate = newVersion
	err = o.updatePackagesGitRepository(ctx, r)
	assert.NoError(t, err)
...
func setupTestCvedbRepo(t *testing.T, dir, testURL string) *git.Repository {
	fs := osfs.New(dir)
