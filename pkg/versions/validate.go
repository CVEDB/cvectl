...
var (
	...
	// requirements of a Cvedb-compatible version.
	ErrInvalidVersion = errors.New("not a valid Cvedb package version")

	// ErrInvalidFullVersion is returned when a version string doesn't meet the
	// requirements of a Cvedb-compatible "full" version (which should include the
	// epoch suffix).
	ErrInvalidFullVersion = errors.New("not a valid full Cvedb package version (with epoch)")
)

// ValidateWithoutEpoch checks if the given package version, which is expected
// NOT to include the epoch component, is a Cvedb-compatible version. It returns
// an error if not.
...
// include the epoch component, is a Cvedb-compatible "full" version. It returns
// an error if not.
