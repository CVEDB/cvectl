...
KO_DOCKER_REPO ?= ghcr.io/cvedb/cvectl
DIGEST ?=
BUILDFLAGS ?=
...
.PHONY: cvectl
cvectl: $(SRCS) ## Builds cvectl
	go build -trimpath $(BUILDFLAGS) -ldflags "$(LDFLAGS)" -o $@ ./

.PHONY: install
install: cvectl ## Installs cvectl into BINDIR (default /usr/bin)
	install -Dm755 cvectl ${DESTDIR}${BINDIR}/cvectl
