...
```
  ...
  -n, --namespace string                 namespace to create the pod in (default "default")
      ...
      --sdk-image string                 sdk image to use (default "ghcr.io/cvedb/sdk:latest")
      ...
      --signing-key-name string          the signing key name to use, the name is important when when signing e.g. keyName=cvedb-signing (default "cvedb-signing")
      --src-bucket string                if set, download contents of packages/* from a location in GCS (default "gs://cvedb-production-registry-destination/os/")
